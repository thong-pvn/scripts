...
# Setup logging cho seed.log
def setup_seed_logging():
    """Setup logging cho seed.log file"""
    log_file = "seed.log"
    logging.basicConfig(
        level=logging.INFO,
...
# Khởi tạo logger
seed_logger = setup_seed_logging()
...
def generate3dObject(prompt: str, pipeline: TrellisTextTo3DPipeline, seed: Optional[int] = None):
    start_time = datetime.now()
    
    if seed is None:
        seed = random.randint(1, 50000)
    
    # Log thông tin bắt đầu generation
    seed_logger.info(f"🚀 GENERATION START - Prompt: '{prompt}' - Seed: {seed}")
    
    result = pipeline.run(
        prompt=prompt,                # Tham số bắt buộc: text prompt
        num_samples=1,                # Số lượng mẫu muốn tạo
        seed=seed,                    # Random seed để tái tạo kết quả
        sparse_structure_sampler_params={
    ...
    seed_logger.info(f"✅ GENERATION COMPLETE - Prompt: '{prompt}' - Seed: {seed} - Duration: {generation_duration:.2f}s")
...
async def generate(prompt: str = Form()) -> Response:
    ...
    outputs = generate3dObject(prompt, trellisPipe)
